...
import vertexShader from "./callisto.vertex.glsl";
import fragmentShader from "./callisto.fragment.glsl";
...
export type CallistoRef = {
  updateSunPositionUniform: (sunPosition: Vector3) => void;
};

const Callisto = forwardRef<CallistoRef, Props>((props, outerRef) => {
  const { geometry } = props;
  const ref = useRef<Mesh<SphereGeometry, ShaderMaterial>>(null);
  const orbit = useRef(new Spherical(ORBIT_RADIUS, Math.PI / 2, -4.5));
